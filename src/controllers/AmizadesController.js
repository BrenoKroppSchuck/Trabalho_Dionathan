...
const JogadoresDAO = require('../models/dao/JogadoresDAO'); // Importando JogadoresDAO
...
class AmizadeController {
    ...
    create(req, res) {
        ...
        const amizadeId = AmizadesDAO.criar(amizade);
    ...
    update(req, res) {
        ...
        const amizadeExistente = AmizadesDAO.buscarPorId(id);
        if (!amizadeExistente) {
            return res.status(404).json({ message: "Amizade não encontrada" });
        }

        // Atualize a amizade no banco de dados
        AmizadesDAO.atualizar(id, amizadeAtualizada);

        // Responda com os detalhes da amizade atualizada
        res.status(200).json({ amizade: AmizadesDAO.buscarPorId(id) });
    }
